...
function initializeMap() {
    ...
    heatLayer = L.heatLayer([], settings).addTo(map);

    // Initialize health display
    healthDisplay = new HealthDisplay('health-display');

    // Add event listeners for map movement
    map.on('moveend', fetchTreeData);
    map.on('zoomend', function() {
        const currentZoom = map.getZoom();
        document.getElementById('health-display').style.display = 
...
async function fetchTreeData() {
    ...
    isLoadingData = true;
    updateLoadingStatus(true, "Loading tree data...");

    try {
        const bounds = map.getBounds();
        const response = await fetch(`/api/tree-data?north=${bounds.getNorth()}&south=${bounds.getSouth()}&east=${bounds.getEast()}&west=${bounds.getWest()}`);
        const data = await response.json();

        // Update heatmap
        heatLayer.setLatLngs(data.locations);

        // Update health display if visible
        if (map.getZoom() < 13) {
            healthDisplay.updateData(data.healthCounts, data.total);
        }

        updateLoadingStatus(false, `Loaded ${data.total.toLocaleString()} trees`);
    } catch (error) {
        console.error('Error fetching tree data:', error);
        updateLoadingStatus(false, "Error loading tree data");
    } finally {
        isLoadingData = false;
    }
}
...
class HealthDisplay {
    ...
    updateData(healthCounts, total) {
        const statsDiv = this.container.querySelector('.health-stats');
        const barsDiv = this.container.querySelector('.health-bars');
        
        // Calculate percentages
        const percentages = {};
        for (const [health, count] of Object.entries(healthCounts)) {
            percentages[health] = ((count / total) * 100).toFixed(1);
        }

        // Update stats
        statsDiv.innerHTML = `
            <div class="total-trees">Total Trees: ${total.toLocaleString()}</div>
            ${Object.entries(healthCounts).map(([health, count]) => `
                <div class="health-stat ${health.toLowerCase()}">
                    <span class="health-label">${health}</span>
                    <span class="health-count">${count.toLocaleString()}</span>
                    <span class="health-percent">(${percentages[health]}%)</span>
                </div>
            `).join('')}
        `;

        // Update bars
        const maxCount = Math.max(...Object.values(healthCounts));
        barsDiv.innerHTML = Object.entries(healthCounts).map(([health, count]) => `
            <div class="health-bar-container">
                <div class="health-bar-fill ${health.toLowerCase()}" 
